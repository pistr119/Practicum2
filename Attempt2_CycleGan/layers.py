#this class defines the layers, which will be used in the model (gen/disc)
...
def instance_norm(x):
    with tf.variable_scope("instance_norm"):
        ...
        scale=tf.get_variable('scale', [x.get_shape()[-1]], initializer=tf.truncated_normal_initializer(mean=1, stddev=.02))
        offset=tf.get_variable('offset', [x.get_shape()[-1]], initializer=tf.constant_initializer(0.0))
        return scale*tf.div(x-mean, tf.sqrt(var+epsilon)) + offset


def general_conv2d(inputconv, o_d=64, f_h=7, f_w=7, s_h=1, s_w=1, stddev=.02, padding="VALID", name="conv2d", do_norm=True, do_relu=True, relufactor=0):
    with tf.variable_scope(name):
